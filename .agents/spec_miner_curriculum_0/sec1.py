# -*- coding: utf-8 -*-
text = """# Specification Mining & Curriculum Requirement Analysis: Missions 006-010
...
- **`B:\\nexus-academy\\types\\mission.types.ts` & `types\\common.types.ts`**:
  - Single schema authority: `MissionData`, `CognitiveLoadEstimate`, `CuriosityBlock`, and 13 fixed steps in order:
    ...
    5. `code_example` (4)
    6. `practice` [Guided] (5)
    7. `practice` [Independent] (6)
    8. `practice` [Transfer] (7)
    9. `debug_challenge` [Bug 1 - Simple] (8)
    10. `debug_challenge` [Bug 2 - Tricky] (9)
    11. `debug_challenge` [Bug 3 - Deep Logic] (10)
    12. `reflection` [Critical Thinking Lab] (11)
    13. `mission_complete` [Summary & Key Learnings] (12)
...
with open("B:/nexus-academy/.agents/spec_miner_curriculum_0/handoff.md", "w", encoding="utf-8") as f:
    f.write(text)
print("Section 1 & 2 written successfully.")
